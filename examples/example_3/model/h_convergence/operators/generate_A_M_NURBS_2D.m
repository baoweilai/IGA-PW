function [A, M] = generate_A_M_NURBS_2D( ...
nurbs_base, nurbs_refine, p_Vr, n_pw_Vr, L, n_gp, Example)

% Assemble the two-dimensional NURBS stiffness and mass matrices.
% Read the geometry and refined-mesh data.
ConPts_o   = nurbs_base.ConPts;
weights_o  = nurbs_base.weights;
knotU_o    = nurbs_base.knotU;
knotV_o    = nurbs_base.knotV;
pu_o       = nurbs_base.pu;
pv_o       = nurbs_base.pv;

Element    = nurbs_refine.Element;
knotU      = nurbs_refine.Ubar;
knotV      = nurbs_refine.Vbar;
Coordinate = nurbs_refine.Coordinate;
NoEs       = nurbs_refine.NoEs;
n_dofs     = nurbs_refine.n_dofs;
pu         = nurbs_refine.pu;
pv         = nurbs_refine.pv;

% Prepare quadrature and sparse matrix triplets.
Fhat = @(x,a,b) ((b-a)*x + a + b) / 2;
[gp, gw] = grule(n_gp);

n_ele_dofs = (pu + 1) * (pv + 1);
Ae = zeros(n_ele_dofs, n_ele_dofs);
Me = zeros(n_ele_dofs, n_ele_dofs);

A_value = zeros(NoEs * n_ele_dofs * n_ele_dofs, 1);
M_value = zeros(NoEs * n_ele_dofs * n_ele_dofs, 1);
row_index = zeros(size(A_value));
column_index = zeros(size(A_value));

global_index = 1;
use_V1 = strcmp(Example, 'Example_1') || strcmp(Example, 'Example_3');

% Integrate the local matrices over each physical element.
for e = 1:NoEs
    Ae(:) = 0;
    Me(:) = 0;

    ue = Coordinate(e,1:2);
    ve = Coordinate(e,3:4);
    J1 = (ue(2) - ue(1)) / 2;
    J2 = (ve(2) - ve(1)) / 2;

    row = Element(e,:);

    for i = 1:n_gp
        u = Fhat(gp(i), ue(1), ue(2));
        uders = bspbasisDers(knotU, pu, u, 1);
        Nu  = uders(1,:).';
        DNu = uders(2,:).';

        for j = 1:n_gp
            v = Fhat(gp(j), ve(1), ve(2));
            vders = bspbasisDers(knotV, pv, v, 1);
            Nv  = vders(1,:);
            DNv = vders(2,:);

            [F, DF] = NurbsSurface(ConPts_o, weights_o, knotU_o, pu_o, u, ...
                knotV_o, pv_o, v);

            basis_funcs = Nu * Nv;
            basis_funcs = basis_funcs(:);

            DNu_v = DNu * Nv;
            DNv_u = Nu  * DNv;

            basis_grad = [DNu_v(:), DNv_u(:)] / DF;
            Jacobian = J1 * J2 * abs(det(DF)) * gw(i) * gw(j);

            Mass_mat_ele = (basis_funcs * basis_funcs.') * Jacobian;

            if use_V1
                V_ext = Vr_2D_Example_1(p_Vr, L, n_pw_Vr, F(1), F(2));
            else
                V_ext = Vr_2D_Example_2(p_Vr, L, n_pw_Vr, F(1), F(2));
            end

            Ae = Ae + (basis_grad * basis_grad.') * Jacobian / 2 + V_ext * Mass_mat_ele;
            Me = Me + Mass_mat_ele;
        end
    end

    % Store the element blocks in global triplets.
    for i1 = 1:n_ele_dofs
        for j1 = 1:n_ele_dofs
            row_index(global_index) = row(i1);
            column_index(global_index) = row(j1);
            A_value(global_index) = Ae(i1,j1);
            M_value(global_index) = Me(i1,j1);
            global_index = global_index + 1;
        end
    end
end

% Assemble and symmetrize the global matrices.
A = sparse(row_index, column_index, A_value, n_dofs, n_dofs);
M = sparse(row_index, column_index, M_value, n_dofs, n_dofs);

A = 0.5 * (A + A');
M = 0.5 * (M + M');
end
