...
export_figure_raster_local(fig, fullfile(resultDir, 'error_fields'), 600);
...
for i = 1:numel(errorFields)
    ...
    export_figure_raster_local(fig, fullfile(resultDir, methodFileNames{i}), 600);
    close_figure_if_valid_local(fig);
end
end
...
caxis(ax, sanitize_color_limits_local([0 cmax]));
