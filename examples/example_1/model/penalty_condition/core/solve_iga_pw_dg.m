...
enabled_variants = get_enabled_variants_local(opts);
run_none = any(strcmpi(enabled_variants, 'None'));
...
if run_none
    ...
    res = struct();
    res.prec_type = 'NoPreconditioner';
    res.time_build_prec = 0;

    t_step = tic;
    [uh, D, rnorms, ~] = call_primme_noprec_local( ...
        Mat, M, n_eigenvalues, primme_target, ops, primme_method);
    ...
    fprintf('[RESULT] None          : lambda1 = %.12f, err1 = %.3e, total = %.6f\n', ...
...
if run_blockdiag
    fprintf('\n---------------- BlockDiag-Jacobi (PRIMME) -----------------\n');

    [bd, time_build_bd] = build_blockdiag_data_local( ...
        Atau, H_pw, M_pw, sigma, tau_shift, diagP, idxI, idxP, eps_diag);

    res = struct();
    res.prec_type = 'BlockDiag-Jacobi';
    res.time_build_prec = time_build_bd;

    precfun = @(x) apply_blockdiag_prec_local(x, bd);
    ...
    fprintf('[RESULT] BlockDiag     : lambda1 = %.12f, err1 = %.3e, total = %.6f\n', ...
        res.lambda(1), res.err1, res.time_total);

    result.BlockDiag = res;
end
...
function [bd, ifb, time_build_prec] = ...
    build_interface_block_prec_data_local( ...
    Atau, P, H_pw, M_pw, sigma, tau_shift, diagP, idxI, idxP, ...
    eps_diag, iface_reg)
...
valid = {'None', 'BlockDiag', 'InterfaceBlock'};
mask = false(size(valid));
for i = 1:numel(valid)
    mask(i) = any(strcmpi(variants, valid{i}));
end
variants = valid(mask);
end
...
if ~isempty(method)
    ...
    [uh, D, rnorms, stats] = primme_eigs(Mat, M, n_eigs, target, ops);
end
end
