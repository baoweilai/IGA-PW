function [P, S] = IGA_DG_Right_Edge_Assemble( ...
nurbs_original, nurbs_refine, pw_index, plane_wave_dofs_index, L, n_dofs)
% Assemble DG interface matrices on the right edge:
%   P : jump-jump term
%   S : jump-average term
...
%% Parametric intervals of right edges
...
%% Loop over right edges
for e = 1:vNoEs
    edge_dofs_plus = right_edge_dofs(e, :);
...
% Geometry and Jacobian on the input patch
        [F, DF_plus] = NurbsSurface( ...
            ConPts_o, weights_o, knotU_o, pu_o, u_right_inner, knotV_o, pv_o, v);

        tau    = DF_plus(:, 2);          % Tangent vector
        ...
        Nu_plus    = Uders_plus(1, end-1:end)';
        DNu_plus   = Uders_plus(2, end-1:end)';

        Vders_plus = bspbasisDers(knotV, pv, v, 1);
        Nv_plus    = Vders_plus(1, :);
        DNv_plus   = Vders_plus(2, :);
