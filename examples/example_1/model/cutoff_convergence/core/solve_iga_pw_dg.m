...
if useRefineCache && exist(opts.refineCacheFile,'file')
    ...
    assert(isfield(S,'pre') && isfield(S.pre,'signature'), ...
        'Invalid refine cache: %s', opts.refineCacheFile);
    sig = S.pre.signature;

    ok = true;
    ok = ok && isfield(sig,'Example')    && strcmp(sig.Example, opts.Example);
    ok = ok && isfield(sig,'L')          && sig.L == L;
    ok = ok && isfield(sig,'a')          && sig.a == a;
    ok = ok && isfield(sig,'Refinement') && sig.Refinement == Refinement;
    ok = ok && isfield(sig,'t')          && sig.t == t;
    ok = ok && isfield(sig,'n_gp')       && sig.n_gp == opts.n_gp;
    ok = ok && isfield(sig,'N_Vr')       && sig.N_Vr == N_Vr;

    if ok
        pre = S.pre;
        cacheHit = true;
    else
        assert(false, 'Refine cache signature does not match: %s', opts.refineCacheFile);
    end
end

if cacheHit
    nurbs_original = pre.nurbs_original;
    nurbs_refine   = pre.nurbs_refine;
    H_nurbs        = pre.H_nurbs;
    M_nurbs        = pre.M_nurbs;
    h              = pre.h;
    n_dofs_nurbs   = pre.n_dofs_nurbs;
    k_Vr           = pre.k_Vr;
    n_pw_Vr        = pre.n_pw_Vr;
    pu             = pre.pu;
    pv             = pre.pv;
    ...
    if useRefineCache
        pre = struct();
        pre.signature = struct();
        pre.signature.Example    = opts.Example;
        pre.signature.L          = L;
        pre.signature.a          = a;
        pre.signature.Refinement = Refinement;
        pre.signature.t          = t;
        pre.signature.pdeg       = 1 + t;
        pre.signature.n_gp       = opts.n_gp;
        pre.signature.N_Vr       = N_Vr;

        pre.nurbs_original = nurbs_original;
        pre.nurbs_refine   = nurbs_refine;
        pre.H_nurbs        = H_nurbs;
        pre.M_nurbs        = M_nurbs;
        pre.h              = h;
        pre.n_dofs_nurbs   = n_dofs_nurbs;
        pre.k_Vr           = k_Vr;
        pre.n_pw_Vr        = n_pw_Vr;
        pre.pu             = pu;
        pre.pv             = pv;

        cacheDir = fileparts(opts.refineCacheFile);
        if ~exist(cacheDir,'dir'), mkdir(cacheDir); end
        save(opts.refineCacheFile,'pre','-v7.3');
    end
end
...
t_solve = tic;
[uh, D, rnorms, stats] = call_primme(Mat, M, n_eigenvalues, targetShift, ops, opts.primme_method, Pfun);
...
fprintf('[PRIMME] Nc=%d refine=%d target=%.1f method=%s\n', ...
    Nc, Refinement, targetShift, string_or_empty(opts.primme_method));
fprintf('        interfaceblock : time = %.6f s\n', meta.time_eigs);
fprintf('        build prec     : %.6f s\n', meta.time_build_prec);
...
% ---------------- Fix global phase using PW conjugate pair (robust) ----------------
...
if ~isempty(method)
    [uh, D, rnorms, stats] = primme_eigs(Mat, M, n_eigs, targetShift, ops, method, Pfun);
else
    [uh, D, rnorms, stats] = primme_eigs(Mat, M, n_eigs, targetShift, ops, [], Pfun);
end
end
