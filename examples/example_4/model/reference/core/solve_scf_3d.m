...
for it = 1:scf_maxit
    ...
    if it == 1
        fprintf(['[reference] SCF %d/%d  lambda=%.15e  abs_dlambda=N/A  ' ...
            'rho=%.3e  tol_eig=%.1e  tol_rho=%.1e  primme_tol=%.1e  iter=%.2fs\n'], ...
            it, scf_maxit, lambda_raw, rho_res, scf_tol_eig, scf_tol_rho, ...
            primme_tol, iter_time);
    else
        fprintf(['[reference] SCF %d/%d  lambda=%.15e  abs_dlambda=%.3e  ' ...
            'rho=%.3e  tol_eig=%.1e  tol_rho=%.1e  primme_tol=%.1e  iter=%.2fs\n'], ...
            it, scf_maxit, lambda_raw, lambda_abs_change, rho_res, ...
            scf_tol_eig, scf_tol_rho, primme_tol, iter_time);
    end
    drawnow;
