...
H.refinement_case_run_file = @refinement_case_run_file_local;
    ...
    function ref = ensure_reference_local()
        % Load or compute the reference case.
    ...
    function runDir = get_case_dir_local(studyName, pdeg, Nc, nElem)
        % Build the result directory for one element-count case.
    ...
    function runDir = get_refinement_case_dir_local(studyName, pdeg, Nc, refineValue)
        % Build the result directory for one refinement case.
    ...
    function opts = build_opts_local(runDir, studyName)
        ...
        opts.preconditioner_type = cfg.preconditioner_type;
...
cfg.iga = struct('fixed_Nc', 20, 'p_list', [1 2], 'Nelement_list', [2, 4, 8, 16]);
cfg.energy = struct('p_list', [1, 2], ...
    'K_list', [2, 4, 8, 12, 16], ...
...
% The workflow adds its required model paths.
...
addpath(fullfile(fileparts(rootDir), 'common', 'operators'), '-begin');
rehash;
end
